...
const UpdatestuCashin = () => {
  ...
  .catch(err => console.log(err))
    ...
    useEffect(()=>{
        axios.get('http://localhost:3001/getstudentcashin/'+id)
        .then(res=>{
            setValues({...values, 
                selectstudent:res.data.selectstudent,
        ...
        const handleUpdate =(e)=>{
            e.preventDefault();
            axios.put('http://localhost:3001/updatecashinstu/'+id,values)
        .then(res=>{
...
<Container fluid className='Add'>
   ...
   <div className="firstbox mt-3">
    ...
    <Link className='bigbtn'><h3 className="reg m-0 ">Update Student CashIn</h3></Link>
    <Link className='bigbtn'>   <p className='tex m-0 '>Cash In > Update Student Cashin</p></Link>
    </div>
    <hr className='hr' />
<br />
...
<Row className='p-4'>
        ...
        <div className="stuId">
           ...
           <Form.Select  type="select" name='selectstudent'  value={values.selectstudent}  onChange={e =>setValues({...values,selectstudent:e.target.value})}>
...
export default UpdatestuCashin