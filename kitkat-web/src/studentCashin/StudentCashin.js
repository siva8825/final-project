...
const handleSubmit=(e)=>{
    e.preventDefault()
    axios.post("http://localhost:3001/studentcashin",values)
    .then(result => {
        alert("StudentCashin Added Successfully")
        navigate('../stucashinlist')
    })
    .catch(err => console.log(err)) 
}
  return (
...
<Container fluid className='Add'>
   ...
   <div className="firstbox mt-3">
    ...
    <Link className='bigbtn'><h3 className="reg m-0 "> Student Cash In</h3></Link>
    <div>
   
   <Link className='text-black' to=''> <Form.Check type="radio" inline aria-label="radio 1" label="Student" name='cashin' /></Link>
   <Link className='text-black' to='../customerCashin'><Form.Check type="radio" inline aria-label="radio 1" label="Customer" name='cashin' /></Link>
...
<Row className='p-4'>
        ...
        <div className="stuId">
           ...
           <Form.Select  type="select" name='selectstudent'  value={values.selectstudent} onChange={(e)=>handleChange(e)} >
